...
$(window).on("scroll", function () {
  ...
  if (scrollTop >= sections.eq(1).offset().top - speed) {
    sections.eq(1).find(".show_up").addClass("active");
  }
  ...
  if (scrollTop >= sections.eq(8).offset().top - speed) {
    ...
    sections.eq(8).find(".show_up").addClass("active");
  }
  if (scrollTop >= sections.eq(9).offset().top - speed) {
    $("header nav ul.gnb li").eq(2).removeClass("active");
    sections.eq(9).find(".show_up").addClass("active");
  }
});
...
$(function () {
  const charts = $(".charts"),
    chart = charts.find(".chart"),
    chartOST = chart.offset().top - 600;
